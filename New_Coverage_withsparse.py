...
def prepare_data(frames, plays_df, max_sequence_length=None):
    ...
    X_sparse = []  # Store sparse matrices instead of dense arrays
    y_coverage_class = []
    ...
    for (gameId, playId, play_frames) in frames:
        ...
        play_sparse_matrices = []
        
        for frame_data in play_frames:
            # Use a sparse matrix for each frame
            sparse_image = lil_matrix((height, width), dtype=np.float32)
            
            for idx, player_data in frame_data.iterrows():
                row = player_data['matrix_row']
                col = player_data['matrix_col']
                if 0 <= row < height and 0 <= col < width:
                    sparse_image[row, col] = 1
            
            play_sparse_matrices.append(sparse_image)
        
        # Skip plays with no frames
        if not play_sparse_matrices:
            continue
            
        X_sparse.append(play_sparse_matrices)
        y_coverage_class.append(coverage_class_int)
    
    # If max_sequence_length is not provided, calculate it
    if max_sequence_length is None:
        max_sequence_length = max(len(seq) for seq in X_sparse)
    
    # Pad sequences and convert to dense format
    num_samples = len(X_sparse)
    X_padded = np.zeros((num_samples, max_sequence_length, height, width), dtype=np.float32)
    
    for i, seq in enumerate(X_sparse):
        seq_len = min(len(seq), max_sequence_length)
        for j in range(seq_len):
            X_padded[i, j, :, :] = seq[j].toarray()  # Convert sparse to dense for training
